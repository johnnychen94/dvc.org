...
- `project` - [OSF](https://osf.io/) project name. If you open a page with your
  project in your browser, you can see it in the address bar :
  `https://osf.io/<your_project>/`.

  ```dvc
  $ dvc remote modify osfremote --local project <project_name>
  ```

  Also it is possible to specify `project` by setting envirement variable
  `OSF_PROJECT`. Run `$ export OSF_PROJECT=<your_project>`
...
- `user` - OSF account username (email address by default):
  ...
  Also it is possible to specify `user` by setting envirement variable
